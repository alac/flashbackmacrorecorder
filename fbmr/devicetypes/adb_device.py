...
class ADBDevice(device.ADBInterfaceDevice):
    ...
    def click(self, x, y):
        ...
        line = ['adb', 'shell', 'input touchscreen tap {0} {1}'.format(str(int(xr)), str(int(yr)))]
    ...
    def swipe(self, x, y, x2, y2, duration):
        ...
        line = ['adb', 'shell', 'input', 'touchscreen',
                'swipe', str(int(xr)), str(int(yr)), str(int(xr2)), str(int(yr2)), str(duration_ms)]
        print('      adbDevice running {0}'.format(line))
        subprocess.check_output(self.insert_flags(line))
