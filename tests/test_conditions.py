...
def test_subimage_condition():
    ...
    state_dict = {}
    assert condition.is_valid(Image.open(
        TESTDATA_ROOT + "contained.png"), state_dict, {}) > 0
    location = state_dict["button"]
    assert condition.is_valid(Image.open(
        TESTDATA_ROOT + "not_contained.png"), state_dict, {}) == 0
    ...
    state_dict = {}
    assert condition2.is_valid(Image.open(
        TESTDATA_ROOT + "contained.png"), state_dict, {}) > 0
    location = state_dict["button"]
    assert condition2.is_valid(Image.open(
        TESTDATA_ROOT + "not_contained.png"), state_dict, {}) == 0
